...
for (var i = 0; i < array_length(upgrade_escolha); i++)
{
    ...
    var _comprado = variable_struct_exists(carta, "comprado") && carta.comprado;
    var _comprar = obj_player.cash >= carta.preco && !_comprado;

    var _x = _inicio_x + i * (_largura + espaco);
    var alvo = 0;

    if (point_in_rectangle(mx, my, _x, _y + entrada_y + saida_y, _x + _largura, _y + _altura + entrada_y + saida_y) && !saindo && !_comprado)
    {
        alvo = 10;
        ...
        if (delay_clike <= 0 && mouse_check_button_pressed(mb_left))
        {
            if (obj_player.cash >= carta.preco)
            {
                ...
                if (variable_struct_exists(carta, "comprado"))
                {
                    carta.comprado = true;
                }
            }
    ...
    if (_comprado)
    {
        draw_sprite_ext(spr_vendeu, 0, _draw_x, _draw_y, _escala, _escala, 0, c_white, 1);
    }

    // Dinheiro insuficiente
    if (!_comprar && !_comprado)
    {
